...
function run
{
    ...
    echo "========================================================================================="
    echo "GitLab -> Project -> Settings -> CI/CD"
    echo ""
    echo "-> General pipelines:"
    echo ""
    echo "  Disable: [ ] Public pipelines"
    echo "  Enable:  [x] Skip outdated deployment jobs"
    echo ""
    echo "-> Variables:"
    echo ""
    echo "  GCP_REGION"
    ...
    echo "  GCP_PROJECT_ID"
    echo "    [x] Protect"
    echo "    Environment scope [dev/staging/prod]"
    echo "    Value: ${GCP_PROJECT_ID}"
    echo "  GCP_BUCKET_CLOUDBUILD_SOURCE"
    echo "    [x] Protect"
    echo "    Environment scope [dev/staging/prod]"
    echo "    Value: ${GCP_BUCKET_CLOUDBUILD_SOURCE}"
    echo "  GCP_BUCKET_CLOUDBUILD_LOG"
    echo "    [x] Protect"
    echo "    Environment scope [dev/staging/prod]"
    echo "    Value: ${GCP_BUCKET_CLOUDBUILD_LOG}"
    echo "  GCP_SERVICEACCOUNT_KEYFILE"
    echo "    Type: [File]"
    echo "    [x] Protect"
    echo "    Environment scope [dev/staging/prod]"
    echo "    Value:"
